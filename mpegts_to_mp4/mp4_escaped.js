	...
	return {
		...
		BoxHeader: {
			...
			_size: jBinary.Template({
				baseType: function (context) { 
					var size = context.size;

                    var bytesCount = 4;
                    
                    var bytes3and2 = size >> 16;
                    var bytes2and1 = (size >> 8) & 0x00FFFF;
                    var bytes1and0 = size & 0x00FFFF;

                    if (bytes3and2 === 0) {
                        ++bytesCount;
                    } else if (bytes2and1 === 0) {
                        return 8 * 5;
                    }
                ...
                read: function(context) {
                    var size = this.binary.read('uint32');
                    
                    var bytes3and2 = context.size >> 16;
                    var bytes2and1 = (context.size >> 8) & 0x00FFFF;
                    var bytes1and0 = context.size & 0x00FFFF;
                    
                    if (bytes3and2 === 0 || bytes2and1 === 0 || bytes1and0 === 0) {
                        throw 'Unsupported escape bytes sequence';
                    }
				...
				write: function (value, context) {
                    ...
                    var escapedSize = bytes3and2;
                    if (bytes3and2 === 0) {
                        escapedSize <<= 8;
                        escapedSize |= 0x03;
                    } else if (bytes2and1 === 0) {
                        var byte1 = bytes2and1 & 0x00FF;
                        var byte0 = bytes1and0 & 0x00FF;
